...
int main() { init(4, 6); Shader::logMaxAttribute();
    ...
    glBindVertexArray(VAO);
        ...
        glBindBuffer(GL_ARRAY_BUFFER, VBO2);
            ...
            ;
            glBufferStorage(GL_ARRAY_BUFFER, cubeamount*sizeof(GLenum), NULL, flags);
            GLenum* dataPtr = (GLenum*)glMapBuffer(GL_ARRAY_BUFFER, GL_WRITE_ONLY);
            GLenum (&blockTypes)[viewSize.x][viewSize.y][viewSize.z] = *reinterpret_cast<GLenum(*)[viewSize.x][viewSize.y][viewSize.z]>(dataPtr);
            ;
    ...
    for(int x = 0; x < viewSize.x; x++) {
        for(int y = 0; y < viewSize.y; y++) {
            for(int z = 0; z < viewSize.z; z++) {
                bool type = !(x==0 || y==0 || z==0 || x==viewSize.x-1 || y==viewSize.y-1 || z==viewSize.z-1);
                //blockTypes[x][y][z] = type;
                dataPtr[x*viewSize.y*viewSize.z + y*viewSize.z + z] = type;
            }
        }
    }
    blockTypes[2][2][2] = 0;
